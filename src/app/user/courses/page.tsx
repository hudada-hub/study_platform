...
function useMyCourses() {
  ...
  useEffect(() => {
    const fetchMyCourses = async () => {
      ...
      try {
        // 查询用户所有订单，聚合唯一课程
        const res = await request('/user/my-courses', { method: 'GET' });
...
export default function CoursesPage() {
  ...
  // 我的课程卡片
  const renderMyCourseCard = (course: any) => (
    <Card key={course.id} className="mb-4">
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="flex-1 p-4">
          ...
          <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm text-gray-600">
            ...
            <div>分类：{course.category?.name}</div>
            <div>方向：{course.direction?.name}</div>
            <div>学习章节数：{course.learnedChapterCount || 0}</div>
            <div>创建时间：{new Date(course.createdAt).toLocaleDateString()}</div>
          </div>
          <div className="mt-4 flex flex-wrap gap-2">
            <Link 
              ...
              <div>
                {myCourses.loading ? (
                  ...
                  <div className="space-y-4 mb-6">
                    {myCourses.courses.map(renderMyCourseCard)}
                  </div>
                ) : (
                  <div className="text-center py-8 text-gray-500">暂无学习课程</div>
                )}
              </div>
            ),
          },
