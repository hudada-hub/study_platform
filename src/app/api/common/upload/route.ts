...
export async function POST(req: NextRequest) {
  try {
    ...
    const strictFormat = searchParams.get('strictFormat') !== 'false'; // 默认true，限制格式

    // 解析上传的文件
    const file = await parseMultipartFormData(req);

    // 检查文件类型（无论是否严格限制都需要用于路径和大小判断）
    ...
    // 如果启用严格格式限制，则验证文件类型
    if (strictFormat && !isImage && !isVideo && !isAudio && !isDocument) {
