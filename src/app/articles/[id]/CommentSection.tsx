...
import { ArrowUpOnSquareIcon, ChatBubbleLeftRightIcon ,HeartIcon} from '@heroicons/react/24/outline';
...
export const CommentSection = ({ articleId,isDetail,wikiId }: CommentSectionProps) => {
    ...
    <div key={comment.id} className={`ml-${depth * 4} mb-6 p-4 bg-white rounded-lg  ${depth > 0 ? 'border-l-4 border-blue-100' : ''}`}>
      ...
      <div className="flex items-center gap-4 text-sm text-gray-500">
        ...
        <button
          ...
          style={{color:comment.hasLiked ?theme.textColor.primary:'#000'}}
          className={`flex items-center gap-1 }`}
        >
          <HeartIcon className={`w-4 h-4 ${comment.hasLiked ? 'fill-current' : ''}`} />
          {comment.likesCount}
        </button>
      </div>
